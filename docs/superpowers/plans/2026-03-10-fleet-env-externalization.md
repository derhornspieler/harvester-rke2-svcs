...
Expected: `KEYCLOAK_FQDN=keycloak.aegisgroup.ch` (or whatever DOMAIN is)
...
- Line 27: `HARBOR="harbor.aegisgroup.ch"` → `HARBOR="${HARBOR_HOST:?Set HARBOR_HOST in .env}"`
- Line 30: `OCI_REGISTRY="oci://${HARBOR}/fleet"` → `OCI_REGISTRY="oci://${HARBOR_HOST}/fleet"`
- Line 5 comment: Replace `oci://harbor.aegisgroup.ch/fleet/` → `oci://<HARBOR_HOST>/fleet/`
...
Line 32: `HARBOR="harbor.aegisgroup.ch"` → `HARBOR="${HARBOR_HOST:?Set HARBOR_HOST in .env}"`
...
Lines 17-18: Replace `harbor.aegisgroup.ch` in comments with `<HARBOR_HOST>`.
...
- Line 36: `HARBOR="harbor.aegisgroup.ch"` → `HARBOR="${HARBOR_HOST:?Set HARBOR_HOST in .env}"`
- Line 231: `192.168.48.2` → `${TRAEFIK_LB_IP}`
- Line 386: `"aegisgroup.ch"` → `"${DOMAIN}"`
...
**Files to modify** (replace `harbor.aegisgroup.ch` with `${HARBOR_HOST}` and `rke2-prod` with `${FLEET_TARGET_CLUSTER}`):
...
- [ ] **Step 1: For each fleet.yaml, replace:**
  - `oci://harbor.aegisgroup.ch/helm/` → `${OCI_HELM_PREFIX}/`
  - `oci://harbor.aegisgroup.ch/fleet/` → `${OCI_FLEET_PREFIX}/`
...
- [ ] **Step 2: Templatize container images in manifests**
  - `00-operators/node-labeler/manifests/deployment.yaml` line 48: `harbor.aegisgroup.ch/library/node-labeler:v0.2.0` → `${IMAGE_NODE_LABELER}`
  - `00-operators/storage-autoscaler/manifests/deployment.yaml` line 48: `harbor.aegisgroup.ch/library/storage-autoscaler:v0.2.0` → `${IMAGE_STORAGE_AUTOSCALER}`
...
- [ ] **Step 4: vault-init-job.yaml**
  - `DOMAIN="aegisgroup.ch"` → `DOMAIN="${DOMAIN}"`
  ...
  **CRITICAL**: This file contains extensive embedded bash. The envsubst explicit variable list in env-defaults.sh ensures only our named variables are substituted. Shell variables like `$VAULT_TOKEN`, `$DOMAIN`, etc. inside the job's `command:` block will NOT be touched because they're not in the ENVSUBST_VARS list — EXCEPT `$DOMAIN` which IS in our list. For the vault-init-job, the `DOMAIN=` assignment inside the job script must use a different approach:
  - Replace `DOMAIN="aegisgroup.ch"` in the env block (line ~85) with `DOMAIN="${DOMAIN}"` — envsubst will substitute this to the actual value at render time. The subsequent usage of `$DOMAIN` inside the script block won't be touched because envsubst only replaces `${DOMAIN}` (with braces), not `$DOMAIN` (without braces).
  ...
  **Simplest approach**: In Job manifests that contain embedded bash, use `CHANGEME_DOMAIN` style tokens that are distinct from shell variable syntax. The render script does `sed` replacements for `CHANGEME_*` tokens.
  ...
  Actually, the cleanest approach: Don't template Job manifests at all. Instead, have the Job scripts read their config from environment variables or ConfigMaps that are already set by the cluster. The vault-init-job already has `DOMAIN="aegisgroup.ch"` as a variable assignment at the top of its script — we just need to change that one assignment.
  ...
  **Final decision**: For Job manifests with embedded bash:
  ...
  2. Move hardcoded values like `DOMAIN="aegisgroup.ch"` to `env:` blocks: `- name: DOMAIN; value: "${DOMAIN}"`
...
- [ ] **Step 3: keycloak gateway.yaml + httproute.yaml**
  - `keycloak.aegisgroup.ch` → `${KEYCLOAK_FQDN}`
...
- [ ] **Step 6: keycloak-config-job.yaml** (embedded bash — use env: block approach)
  - Move `DOMAIN="aegisgroup.ch"` to container `env:` section
...
- `grafana.aegisgroup.ch` → `${GRAFANA_FQDN}`
...
- [ ] **Step 8: Grafana dashboard configmaps with hardcoded domains**
  - `configmap-dashboard-firing-alerts.yaml` — `alertmanager.aegisgroup.ch` → `${ALERTMANAGER_FQDN}`
...
- `harbor/manifests/gateway.yaml + httproute.yaml` — FQDN (harbor.dev.aegisgroup.ch)
...
Harbor-specific: `harbor.dev.aegisgroup.ch` → `${HARBOR_FQDN}` (which defaults to `${HARBOR_HOST}`)
...
- [ ] **Step 1: argocd/values.yaml** — This is the most complex single file
  - `domain: argo.aegisgroup.ch` → `domain: ${ARGOCD_FQDN}`
  - `url: https://argo.aegisgroup.ch` → `url: https://${ARGOCD_FQDN}`
...
- [ ] **Step 1: Search and replace comments referencing aegisgroup.ch**
...
|----------|--------|-------|
| Domain references | 130+ hardcoded `aegisgroup.ch` | `${DOMAIN}` + derived FQDNs |
