...
|----------|-------|---------|-----------|
| `gitlab-ci` | `gitlab-ci@aegisgroup.ch` | CI pipeline operations: owns SSH deploy key on `platform/platform-deployments` (can_push=true), Developer access on `platform` group | `keycloak-config` Job (Phase 2c) |
