...
graph TD
    ...
    class MinIO,BackupMinIO storage
...
### Backup MinIO (Cross-Cluster Backups)

A dedicated MinIO instance at `backup.<DOMAIN>` provides S3-compatible object storage for cross-cluster backup consumers. This is separate from the shared MinIO (which stores CNPG WAL archives, Harbor blobs, and GitLab artifacts) to isolate backup traffic and credentials.

**Architecture:**
- S3-only endpoint (no MinIO Console UI)
- TLS via Vault PKI (cert-manager leaf certificate)
- Gateway API with HTTPRoute for external access
- VolumeAutoscaler: 50Gi initial, scales to 2Ti max (80% threshold, 25% increase)

**Buckets:**

| Bucket | Consumer | Purpose |
|--------|----------|---------|
| `rancher-backups` | Rancher Backup Operator | Rancher management cluster backups |
| `velero` | Velero | Kubernetes resource and PV backups |
| `etcd-snapshots` | etcd snapshot tool | etcd cluster state snapshots |

**Credential Model:**
- Root credentials seeded by `vault-init` at `kv/services/backup/root-credentials`
- Per-consumer scoped access keys stored at `kv/services/backup/<consumer>` (e.g., `kv/services/backup/rancher-backup`)
- `backup-init` Job creates buckets and per-consumer access keys with scoped policies
- ExternalSecret reads per-consumer credentials for each backup tool

**Bundle Structure (35-backup, 3 bundles):**

| Bundle | Name | Purpose |
|--------|------|---------|
| backup-minio | MinIO server deployment | S3-compatible object storage |
| backup-init | Bucket and credential provisioning | Creates buckets and per-consumer access keys |
| backup-manifests | Gateway, HTTPRoute, VolumeAutoscaler | Ingress and storage autoscaling |
