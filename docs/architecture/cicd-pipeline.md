...
## Platform CI Pipeline (This Repository)

The CI pipeline for the `harvester-rke2-svcs` repository itself uses a reduced set of active stages. The full deploy stages (build, validate, push, deploy) are **disabled** (job names prefixed with `.` to make them hidden jobs in GitLab CI).

### Active CI Stages

| Stage | Jobs | Purpose |
|-------|------|---------|
| `lint` | shellcheck, yamllint, helm-lint | Static analysis of shell scripts, YAML, and Helm charts |
| `sync` | sync-to-github | Mirror sanitized history to GitHub (shadow repo, single orphan commit) |

### Disabled CI Stages

| Stage | Status | Reason |
|-------|--------|--------|
| `validate` | Disabled (`.validate-*`) | CI full deploy broke cluster by deploying ALL groups simultaneously |
| `push-bundles` | Disabled (`.push-bundles`) | Same root cause — requires group-by-group ordering |
| `deploy` | Disabled (`.deploy-fleet`) | Fleet HelmOps must deploy in dependency order, not all at once |

### Current Deployment Workflow

Workstation-driven deploys via `deploy-fleet-helmops.sh --group <group>` are the active workflow. The CI deploy pipeline will be re-enabled once group-by-group deployment ordering is implemented in CI.

```bash
# Typical operator workflow (from workstation)
...
The `sync-to-github` job uses a **shadow repo approach**: it pushes a single orphan commit with the current tree snapshot. No Git history or diffs are exposed on GitHub. This ensures the GitHub mirror contains only the latest state of the codebase.
