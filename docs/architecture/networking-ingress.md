...
- Users request services by hostname (e.g., `keycloak.dev.aegisgroup.ch`)
...
When a user opens their browser to `https://grafana.dev.aegisgroup.ch/`:

1. **DNS resolution**: User's DNS resolver asks "where is grafana.dev.aegisgroup.ch?" and gets back the cluster's public IP (managed externally, not shown in diagram)

2. **Traefik receives the request**: The request arrives at Traefik (running on every node as a DaemonSet). Traefik listens on port 443 (standard HTTPS) and sees the hostname `grafana.dev.aegisgroup.ch`.
...
4. **HTTPRoute decides destination**: An HTTPRoute rule says "if hostname is `grafana.dev.aegisgroup.ch`, forward to the Grafana service on port 3000 inside the cluster."
...
**Special case — GitLab SSH**: When a developer runs `git clone git@gitlab.dev.aegisgroup.ch:namespace/project.git`, the request goes to port 2222 (not 443). Traefik sees TCP traffic on port 2222 and uses a TCPRoute to forward directly to the GitLab SSH service, no HTTP parsing needed.
...
|---------|-----|----------|-----------------|
| Keycloak (OAuth2 Provider) | `https://keycloak.dev.aegisgroup.ch` | HTTPS | Public OAuth2 server |
| Prometheus | `https://prometheus.dev.aegisgroup.ch` | HTTPS | OAuth2-proxy (Keycloak) |
| Alertmanager | `https://alertmanager.dev.aegisgroup.ch` | HTTPS | OAuth2-proxy (Keycloak) |
| Grafana | `https://grafana.dev.aegisgroup.ch` | HTTPS | OAuth2-proxy (Keycloak) |
| Hubble (Cilium Network Visibility) | `https://hubble.dev.aegisgroup.ch` | HTTPS | OAuth2-proxy (Keycloak) |
| Harbor (Container Registry) | `https://harbor.dev.aegisgroup.ch` | HTTPS | Basic auth (users/passwords) |
| ArgoCD | `https://argo.dev.aegisgroup.ch` | HTTPS | OAuth2-proxy (Keycloak PKCE) |
| Argo Rollouts | `https://argo-rollouts.dev.aegisgroup.ch` | HTTPS | OAuth2-proxy (Keycloak) |
| Argo Workflows | `https://argo-workflows.dev.aegisgroup.ch` | HTTPS | OAuth2-proxy (Keycloak) |
| GitLab Web | `https://gitlab.dev.aegisgroup.ch` | HTTPS | OAuth2 / SAML (Keycloak) |
| GitLab SSH | `git@gitlab.dev.aegisgroup.ch:22` (port 2222) | SSH/TCP | SSH key authentication |
| Vault | `https://vault.dev.aegisgroup.ch` | HTTPS | OAuth2-proxy (Keycloak) |
| Traefik Dashboard | `https://traefik.dev.aegisgroup.ch` | HTTPS | OAuth2-proxy (Keycloak) |
...
- Listens on a hostname (e.g., `keycloak.dev.aegisgroup.ch`)
...
spec:
  ...
  listeners:
    - name: https
      ...
      hostname: example.dev.aegisgroup.ch     # Matches HTTPRoute
      tls:
        mode: Terminate
        certificateRefs:
...
### Cluster Ingress IP
The cluster has a single public IP address that all `*.dev.aegisgroup.ch` domains point to via DNS A records. External load balancing (DNS round-robin or cloud LB) is outside the scope of this document.
...
5. Test from inside the cluster: `curl -k https://service.dev.aegisgroup.ch`
