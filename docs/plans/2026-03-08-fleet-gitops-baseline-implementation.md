...
- **Helm charts**: Reference OCI URLs in Harbor (`oci://harbor.aegisgroup.ch/helm/<chart>`). Values files copied from existing `services/<svc>/*-values.yaml`.
...
- **Harbor OCI registry**: `harbor.aegisgroup.ch`
- **Domain**: `aegisgroup.ch` (substitute via `CHANGEME_DOMAIN` tokens where needed)
...
helm:
  releaseName: cnpg
  chart: oci://harbor.aegisgroup.ch/helm/cloudnative-pg
  version: "0.27.1"
  valuesFiles:
    - values.yaml
targets:
...
helm:
  ...
  chart: oci://harbor.aegisgroup.ch/helm/redis-operator
  version: "0.23.0"
  valuesFiles:
    - values.yaml
targets:
...
```bash
sed -e 's/${harbor_fqdn}/harbor.aegisgroup.ch/g' \
...
helm:
  ...
  chart: oci://harbor.aegisgroup.ch/helm/cert-manager
  version: "v1.19.4"
  valuesFiles:
    - values.yaml
targets:
...
helm:
  releaseName: vault
  chart: oci://harbor.aegisgroup.ch/helm/vault
  version: "0.32.0"
  valuesFiles:
    - values.yaml
targets:
...
helm:
  ...
  chart: oci://harbor.aegisgroup.ch/helm/external-secrets
  version: "2.0.1"
  valuesFiles:
    - values.yaml
dependsOn:
...
- Add `alice.morgan` as super-admin
...
helm:
  ...
  chart: oci://harbor.aegisgroup.ch/helm/kube-prometheus-stack
  version: "82.10.0"
  valuesFiles:
    - values.yaml
targets:
...
helm:
  releaseName: harbor
  chart: oci://harbor.aegisgroup.ch/helm/harbor
  version: "1.18.2"
  valuesFiles:
    - values.yaml
dependsOn:
...
helm:
  releaseName: gitlab
  chart: oci://harbor.aegisgroup.ch/helm/gitlab
  version: "9.9.2"
  valuesFiles:
    - values.yaml
dependsOn:
