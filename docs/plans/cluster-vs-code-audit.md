...
**Status**: COMPLETE
**Environment**: AIRGAPPED — cluster has NO internet access. ALL container images, Helm charts, RPM repos, and RKE2 binaries must be sourced via Harbor proxy-cache (`harbor.aegisgroup.ch`). Any direct pull from quay.io, ghcr.io, docker.io, registry.k8s.io, or GitHub raw URLs will **FAIL** on a fresh deploy.
...
| ArgoCD TLS certs CM | `keycloak.aegisgroup.ch` CA trust in `argocd-tls-certs-cm` not in Helm values |
...
| A14 | RESOLVED | 1 | ~~Route Vault/cert-manager/ESO images through Harbor~~ — RKE2 `registries.yaml` rewrites all image pulls through `harbor.aegisgroup.ch` at containerd level. No YAML changes needed. | N/A |
...
| A49 | MEDIUM | 5 | Add `configs.tls.certificates` for keycloak.aegisgroup.ch CA trust in ArgoCD values | TODO |
...
| A78 | HIGH | 5 | ArgoCD/Rollouts/Workflows OCI charts from `oci://ghcr.io/argoproj/...` — `.env.example` has `HELM_CHART_*` overrides (lines 75-77). Ensure user sets these for airgap to point to Harbor OCI proxy or charts.aegisgroup.ch-synced artifacts | TODO |
| A79 | HIGH | 4 | Harbor OCI chart from `oci://registry-1.docker.io/goharbor/...` — `.env.example` has override (line 52). Ensure user sets for airgap. Already synced via charts.aegisgroup.ch | TODO |
| A80 | RESOLVED | all | ~~Container image registry rewrite~~ — RKE2 `registries.yaml` mirrors all upstream registries (docker.io, quay.io, ghcr.io, registry.k8s.io, gcr.io, docker.elastic.co, registry.gitlab.com) through `harbor.aegisgroup.ch` with rewrite rules at containerd level. No YAML changes needed. | N/A |
...
> **Note:** RKE2 `registries.yaml` on all nodes mirrors docker.io, quay.io, ghcr.io, registry.k8s.io, gcr.io, docker.elastic.co, and registry.gitlab.com through `harbor.aegisgroup.ch` at the containerd level. Container image references in YAML manifests do NOT need rewriting. Helm chart repos use `.env` overrides (`HELM_REPO_*`/`HELM_CHART_*`) for airgap.
...
|-------------------|---------|----------|
| `harbor.aegisgroup.ch` | Container images (proxy-cache) + OCI Helm charts | docker.io, quay.io, ghcr.io, registry.k8s.io |
| `charts.aegisgroup.ch` | HTTP Helm chart repos (nginx proxy + helm-sync to Harbor OCI) | jetstack, hashicorp, prometheus-community, etc. |
| `yum.aegisgroup.ch` | Rocky 9 RPM repos, EPEL, RKE2 RPMs | mirrorlist.rockylinux.org, etc. |
...
| quay.io | harbor.aegisgroup.ch | `quay.io/$1` |
| ghcr.io | harbor.aegisgroup.ch | `ghcr.io/$1` |
| registry.k8s.io | harbor.aegisgroup.ch | `registry.k8s.io/$1` |
| gcr.io | harbor.aegisgroup.ch | `gcr.io/$1` |
| docker.elastic.co | harbor.aegisgroup.ch | `docker.elastic.co/$1` |
| registry.gitlab.com | harbor.aegisgroup.ch | `registry.gitlab.com/$1` |
| docker-registry3.mariadb.com | harbor.aegisgroup.ch | `docker-registry3.mariadb.com/$1` |
...
| `https://helm.releases.hashicorp.com` | `https://charts.aegisgroup.ch/hashicorp` |
| `https://charts.external-secrets.io` | `https://charts.aegisgroup.ch/external-secrets` |
| `https://helm.goharbor.io` | `https://charts.aegisgroup.ch/goharbor` |
| `https://prometheus-community.github.io/helm-charts` | `https://charts.aegisgroup.ch/prometheus-community` |
| `https://cloudnative-pg.github.io/charts` | `https://charts.aegisgroup.ch/cnpg` |
