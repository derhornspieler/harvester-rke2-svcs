...
and the same domain. If dev and prod apps both use `svc-forge.example.com`,
...
|-------------|-------------|---------|-----|
| Dev | `<app>.dev.example.com` | `svc-forge.dev.example.com` | `*.dev.example.com` → Traefik LB IP |
| Staging | `<app>.staging.example.com` | `svc-forge.staging.example.com` | `*.staging.example.com` → Traefik LB IP |
| Production | `<app>.example.com` | `svc-forge.example.com` | `*.example.com` → Traefik LB IP (existing) |
...
ExternalDNS already manages `*.example.com` via RFC2136. Add two wildcard
records for the environment subdomains:

```
*.dev.example.com      A    192.168.48.2
*.staging.example.com  A    192.168.48.2
```
...
cert-manager's `vault-issuer` ClusterIssuer already covers `example.com`
...
- `svc-forge.dev.example.com` → cert-manager auto-issues via vault-issuer
- `svc-forge.staging.example.com` → cert-manager auto-issues via vault-issuer
- `svc-forge.example.com` → cert-manager auto-issues via vault-issuer
  ...
  - target:
    ...
    patch: |
      - op: replace
        ...
        value: svc-forge.dev.example.com
...
patches:
  - target:
    ...
    patch: |
      - op: replace
        path: /spec/hostnames/0
        value: svc-forge.staging.example.com
...
1. **Smoke tests in dev**: CI can `curl svc-forge.dev.example.com/health`
   after deploy to verify the app is running
2. **Integration tests in staging**: Full test suite runs against
   `svc-forge.staging.example.com` with realistic traffic
...
1. Add DNS wildcard records for `*.dev.example.com` and `*.staging.example.com`
