...
**Issue**: Service README and documentation referenced `hubble.dev.aegisgroup.ch` but live cluster uses `hubble.aegisgroup.ch` (no `dev.` prefix).
...
# Confirmed: hubble.aegisgroup.ch (no dev. prefix)
```
