...
## Multi-Cluster Deployments (BUNDLE_PREFIX)

The deployment scripts support a `BUNDLE_PREFIX` variable for running multiple clusters (prod + test) side by side. Environment configs live in `.env.rke2-prod` and `.env.rke2-test`.

To deploy to a test cluster, swap the active `.env` file before running scripts:

```bash
cp .env.rke2-test .env          # switch to test config (BUNDLE_PREFIX='test-')
./push-bundles.sh               # pushes prefixed bundles to Harbor
./deploy-fleet-helmops.sh       # creates test-* HelmOps in fleet-default
cp .env.rke2-prod .env          # restore prod config when done
```

Prefixed bundles coexist with production in the same `fleet-default` namespace. Upstream Helm charts (`/helm/`) are shared; only raw bundle OCI paths (`/fleet/`) are prefixed.
