...
ArgoCD is deployed in HA mode (2 controller replicas, 2+ server replicas, redis-ha with 3 replicas) and is accessible at `https://argo.example.com`. Authentication is via Keycloak OIDC -- the `platform-admins` and `infra-engineers` groups get admin access, while `developers` and `senior-developers` get read-only.
...
6. Creates a `gitlab-repo-creds` Secret in the `argocd` namespace (labeled `argocd.argoproj.io/secret-type: repo-creds`) so ArgoCD can pull from any `https://gitlab.example.com/*` repo.
...
spec:
  project: default
  source:
    repoURL: https://gitlab.example.com/apps/my-service.git
    targetRevision: main
    path: deploy/
    helm:
      valueFiles:
...
- `source.repoURL` -- must be a GitLab repo under `https://gitlab.example.com/`. The template credential covers all repos.
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: my-service
          image: harbor.example.com/apps/my-service:v1.2.3
```
...
Or use the ArgoCD UI at `https://argo.example.com` to view sync history and roll back to a previous state.
