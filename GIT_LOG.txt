...
- Replace aegisgroup.ch with example.com in all scripts and docs
- Replace "Aegis Group" org name with "My Organization" (configurable via -o flag and ORG env var)
...
- setup-keycloak.sh Phase 2: create PLATFORM_ADMIN_USER (default: alice.morgan)
...
- Fix KC_REALM from "Aegis Group" to "platform" in .env
...
- Add Hubble hostname correction: hubble.dev.aegisgroup.ch → hubble.aegisgroup.ch
...
- Use upstream image refs for CNPG PostgreSQL (ghcr.io/cloudnative-pg/)
  instead of hardcoded harbor.aegisgroup.ch/proxy-ghcr/ — RKE2
...
Co-Authored-By: Claude Opus 4.6 <noreply@anthropic.com>
---
26d4764 feat: grant alice.morgan admin role in master realm for API access
...
- alice.morgan as primary admin in master realm
