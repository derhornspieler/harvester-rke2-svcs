...
- Hostname: `hubble.example.com` (no `dev.` prefix for production environment)
...
2. **Bundle 2 (Identity)**:
   ...
   - Hubble OIDC client created (redirect_uri: `https://hubble.example.com/oauth2/callback`)
...
5. **Test access**:
   ```bash
   curl -k https://hubble.example.com/
...
**Symptom**: Requests to `https://hubble.example.com` return 503 or connection refused
...
To use a different hostname (e.g., `network-visibility.example.com`):
