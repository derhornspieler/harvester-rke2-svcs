...
- Hostname: `hubble.<DOMAIN>` (no `dev.` prefix for production environment)
...
2. **Bundle 2 (Identity)**:
   ...
   - Hubble OIDC client created (redirect_uri: `https://hubble.<DOMAIN>/oauth2/callback`)
...
**Symptom**: Requests to `https://hubble.<DOMAIN>` return 503 or connection refused
...
kubectl -n keycloak exec -it <keycloak-pod> -- \
  bash -c 'cd /opt/keycloak && bin/kcadm.sh get clients -r platform --username admin'
```
...
To use a different hostname (e.g., `network-visibility.<DOMAIN>`):
