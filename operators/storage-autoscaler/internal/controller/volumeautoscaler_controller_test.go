...
var _ = Describe("VolumeAutoscaler Controller", func() {
	...
	Context("When expanding via controllerRef", Ordered, func() {
		...
		It("should sync CNPG replica PVCs to match expanded size", func() {
			Expect(k8sClient.Create(ctx, cnpgCluster)).To(Succeed())

			// Set status.phase to healthy
			cnpgCluster.Object["status"] = map[string]any{
				"phase": "Cluster in healthy state",
			}
			Expect(k8sClient.Status().Update(ctx, cnpgCluster)).To(Succeed())

			// Create an expandable StorageClass
			scName := "expandable-sc-sync"
			sc := &storagev1.StorageClass{
				ObjectMeta: metav1.ObjectMeta{
					Name: scName,
				},
				Provisioner:          "fake.csi.test",
			...
			// Create 3 PVCs with cnpg.io/cluster label, all at 5Gi
			// PVCs must be Bound for the API server to allow spec.resources.requests changes
			cnpgLabels := map[string]string{
				"cnpg.io/cluster": "test-cnpg-cluster",
			}
			pvcNames := []string{"test-cnpg-cluster-1", "test-cnpg-cluster-2", "test-cnpg-cluster-3"}
			var createdPVCs []corev1.PersistentVolumeClaim
			for idx, name := range pvcNames {
				pvName := fmt.Sprintf("test-pv-cnpg-%d", idx)
				// Create a PV for binding
			...
			va := buildVA("test-cnpg-cluster")
			va.Spec.Target.Selector = &metav1.LabelSelector{
				MatchLabels: cnpgLabels,
			}

			// Simulate: PVC-1 was expanded (it's in the expandedPVCs list)
			targetSize := resource.MustParse("15Gi")
			err := reconciler.syncCNPGReplicaPVCs(ctx, va, createdPVCs[:1], targetSize)
			Expect(err).NotTo(HaveOccurred())

			// Verify PVC-2 and PVC-3 are expanded to 15Gi
			for _, name := range pvcNames[1:] {
				updated := &corev1.PersistentVolumeClaim{}
				Expect(k8sClient.Get(ctx, types.NamespacedName{Name: name, Namespace: vaNamespace}, updated)).To(Succeed())
				requestedSize := updated.Spec.Resources.Requests[corev1.ResourceStorage]
				Expect(requestedSize.Cmp(targetSize)).To(Equal(0),
					"PVC %s should be expanded to %s but got %s", name, targetSize.String(), requestedSize.String())
			}

			// Verify PVC-1 was NOT re-patched (it was in the expanded set)
			pvc1 := &corev1.PersistentVolumeClaim{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: pvcNames[0], Namespace: vaNamespace}, pvc1)).To(Succeed())
			pvc1Size := pvc1.Spec.Resources.Requests[corev1.ResourceStorage]
			originalSize := resource.MustParse("5Gi")
			Expect(pvc1Size.Cmp(originalSize)).To(Equal(0),
				"PVC-1 should remain at original size since it was in expanded set (controller handles it)")
		})

		It("should skip replica PVCs already at or above target size", func() {
			// Use a separate CNPG cluster to avoid label collisions with other tests
			skipCluster := &unstructured.Unstructured{}
			skipCluster.SetGroupVersionKind(cnpgGVK)
			skipCluster.SetName("skip-cnpg-cluster")
			skipCluster.SetNamespace(vaNamespace)
			Expect(unstructured.SetNestedField(skipCluster.Object, "10Gi", "spec", "storage", "size")).To(Succeed())
			Expect(k8sClient.Create(ctx, skipCluster)).To(Succeed())
			defer func() {
				_ = k8sClient.Delete(ctx, skipCluster)
			}()

			// Create an expandable StorageClass
			scName := "expandable-sc-skip"
			sc := &storagev1.StorageClass{
				ObjectMeta: metav1.ObjectMeta{
					Name: scName,
				},
			...
			cnpgLabels := map[string]string{
				"cnpg.io/cluster": "skip-cnpg-cluster",
			}
			// PVC-1: 5Gi (expanded), PVC-2: 5Gi (needs sync), PVC-3: 20Gi (already larger)
			pvcSpecs := []struct {
				name string
				size string
			}{
			...
			var createdPVCs []corev1.PersistentVolumeClaim
			for idx, spec := range pvcSpecs {
				pvName := fmt.Sprintf("test-pv-skip-%d", idx)
				pv := &corev1.PersistentVolume{
					ObjectMeta: metav1.ObjectMeta{
						Name: pvName,
					},
			...
			defer func() {
				for idx, spec := range pvcSpecs {
					p := &corev1.PersistentVolumeClaim{}
					if err := k8sClient.Get(ctx, types.NamespacedName{Name: spec.name, Namespace: vaNamespace}, p); err == nil {
						_ = k8sClient.Delete(ctx, p)
					}
					pv := &corev1.PersistentVolume{}
					pvName := fmt.Sprintf("test-pv-skip-%d", idx)
			...
			va := buildVA("skip-cnpg-cluster")
			targetSize := resource.MustParse("15Gi")
			err := reconciler.syncCNPGReplicaPVCs(ctx, va, createdPVCs[:1], targetSize)
			Expect(err).NotTo(HaveOccurred())

			// PVC-2 should be expanded to 15Gi
			pvc2 := &corev1.PersistentVolumeClaim{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: "sync-skip-2", Namespace: vaNamespace}, pvc2)).To(Succeed())
			pvc2Size := pvc2.Spec.Resources.Requests[corev1.ResourceStorage]
			Expect(pvc2Size.Cmp(targetSize)).To(Equal(0))

			// PVC-3 should remain at 20Gi (already larger)
			pvc3 := &corev1.PersistentVolumeClaim{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: "sync-skip-3", Namespace: vaNamespace}, pvc3)).To(Succeed())
			largerSize := resource.MustParse("20Gi")
			pvc3Size := pvc3.Spec.Resources.Requests[corev1.ResourceStorage]
			Expect(pvc3Size.Cmp(largerSize)).To(Equal(0))
		})
