...
const (
	...
	// CNPG controller reference constants
...
func (r *VolumeAutoscalerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// 1b. Reconcile CNPG PVC size mismatches (runs every loop, fast no-op if aligned)
	if err := r.reconcileCNPGPVCSizes(ctx, &va); err != nil {
		log.Error(err, "CNPG PVC size reconciliation failed (non-fatal)")
	}
...
func (r *VolumeAutoscalerReconciler) expandViaController(
	...
	if ref.APIVersion != cnpgAPIVersion || ref.Kind != cnpgKind {
...
func (r *VolumeAutoscalerReconciler) syncCNPGReplicaPVCs(
	...
	if va.Spec.Target.ControllerRef.APIVersion != cnpgAPIVersion ||
		va.Spec.Target.ControllerRef.Kind != cnpgKind {
		return nil
	}
...
// reconcileCNPGPVCSizes checks for PVC size mismatches in CNPG clusters
// and expands smaller PVCs to match the largest one. This handles existing
// mismatches from before v0.5.1 and cases where Fleet reverted the Cluster
// spec.storage.size after an autoscaler expansion.
func (r *VolumeAutoscalerReconciler) reconcileCNPGPVCSizes(
	ctx context.Context,
	va *autoscalingv1alpha1.VolumeAutoscaler,
) error {
	if va.Spec.Target.ControllerRef == nil {
	...
	// List all PVCs for this CNPG cluster
	var pvcList corev1.PersistentVolumeClaimList
	if err := r.List(ctx, &pvcList,
		client.InNamespace(va.Namespace),
		client.MatchingLabels{"cnpg.io/cluster": clusterName},
	); err != nil {
		return fmt.Errorf("failed to list CNPG PVCs for %s: %w", clusterName, err)
	}

	if len(pvcList.Items) < 2 {
		return nil // Single instance, nothing to sync
	}

	// Find the maximum actual capacity across all PVCs
	var maxSize resource.Quantity
	for _, pvc := range pvcList.Items {
		size := pvc.Status.Capacity[corev1.ResourceStorage]
		if size.Cmp(maxSize) > 0 {
			maxSize = size.DeepCopy()
		}
	}
	...
	// Expand any PVCs whose request AND capacity are both below max
	for i := range pvcList.Items {
		pvc := &pvcList.Items[i]
		currentCapacity := pvc.Status.Capacity[corev1.ResourceStorage]
		currentRequest := pvc.Spec.Resources.Requests[corev1.ResourceStorage]

		// Skip if capacity already matches (or exceeds)
		if currentCapacity.Cmp(maxSize) >= 0 {
			continue
		}
		// Skip if request is already at target (expansion in progress)
		if currentRequest.Cmp(maxSize) >= 0 {
			continue
		}

		log.Info("reconciling CNPG PVC size mismatch",
			"pvc", pvc.Name, "cluster", clusterName,
			"current", currentCapacity.String(), "target", maxSize.String())

		patch := client.MergeFrom(pvc.DeepCopy())
		pvc.Spec.Resources.Requests[corev1.ResourceStorage] = maxSize.DeepCopy()
		if err := r.Patch(ctx, pvc, patch); err != nil {
			log.Error(err, "failed to reconcile PVC size", "pvc", pvc.Name)
			continue
		}

		r.Recorder.Eventf(va, nil, corev1.EventTypeNormal, "ReconcilePVCSize", "AlignReplicaPVC",
			"Reconciled CNPG PVC %s/%s from %s to %s (cluster %s)",
			pvc.Namespace, pvc.Name, currentCapacity.String(), maxSize.String(), clusterName)
		appmetrics.ScaleEventsTotal.WithLabelValues(pvc.Namespace, pvc.Name, va.Name).Inc()
	}
