...
#   ./deploy.sh --env .env.rke2-test   # Use a specific environment file
#
# Prerequisites:
...
# Build --env passthrough args for sub-scripts
ENV_ARGS=()
...
push_charts() {
  ...
  bash "${push_script}" "${ENV_ARGS[@]+"${ENV_ARGS[@]}"}"
...
deploy_helmops() {
  ...
  bash "${deploy_script}" "${ENV_ARGS[@]+"${ENV_ARGS[@]}"}" "${args[@]+"${args[@]}"}"
}
...
seed_manual_secrets() {
  ...
  elif [[ -n "${GITLAB_LICENSE_CONTENT:-}" ]]; then
    vexec kv put kv/services/gitlab license="${GITLAB_LICENSE_CONTENT}" activation-code="" license-encryption-key="${GITLAB_LICENSE_KEY_CONTENT:-}"
    log_ok "GitLab license (file) seeded in Vault at services/gitlab"
  elif [[ -n "${GITLAB_LICENSE:-}" ]]; then
    vexec kv put kv/services/gitlab activation-code="${GITLAB_LICENSE}" license="" license-encryption-key="${GITLAB_LICENSE_KEY_CONTENT:-}"
    log_ok "GitLab license seeded in Vault at services/gitlab"
  else
    vexec kv put kv/services/gitlab activation-code="" license="" license-encryption-key=""
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "                     Example: ./deploy.sh --env .env.rke2-test"
...
if [[ "${STATUS_MODE}" == true ]]; then
  ...
  bash "${SCRIPT_DIR}/deploy-fleet-helmops.sh" "${ENV_ARGS[@]+"${ENV_ARGS[@]}"}" "${status_args[@]}"
  exit 0
fi

if [[ "${DELETE_MODE}" == true ]]; then
  bash "${SCRIPT_DIR}/deploy-fleet-helmops.sh" "${ENV_ARGS[@]+"${ENV_ARGS[@]}"}" --delete
  exit 0
fi
...
# Phase 2: Only seed Root CA for 05-pki-secrets or full deploy
if [[ -z "${SINGLE_GROUP}" || "${SINGLE_GROUP}" == "05-pki-secrets" ]]; then
  seed_root_ca
else
  log_info "Skipping Root CA seeding (not needed for --group ${SINGLE_GROUP})"
fi

deploy_helmops

# Phase 5: Only sign Vault CSR for 05-pki-secrets or full deploy
if [[ -z "${SINGLE_GROUP}" || "${SINGLE_GROUP}" == "05-pki-secrets" ]]; then
  sign_intermediate_csr
else
  log_info "Skipping Vault CSR signing (not needed for --group ${SINGLE_GROUP})"
fi

# Phase 6: Only seed manual secrets for 50-gitlab, 60-cicd-onboard, or full deploy
if [[ -z "${SINGLE_GROUP}" || "${SINGLE_GROUP}" == "50-gitlab" || "${SINGLE_GROUP}" == "60-cicd-onboard" ]]; then
  seed_manual_secrets
else
  log_info "Skipping manual secret seeding (not needed for --group ${SINGLE_GROUP})"
fi
