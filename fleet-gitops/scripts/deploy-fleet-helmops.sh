...
#   ./deploy-fleet-helmops.sh --env .env.rke2-test   # Use a specific environment file
#
# Prerequisites:
...
# Source environment (supports --env <file> override)
...
HELMOP_DEFS=(
  ...
  "identity-cnpg-keycloak|oci://${HARBOR}/fleet/identity-cnpg-keycloak|${BUNDLE_VERSION}|database|identity-cnpg-keycloak|pki-vault-bootstrap-store,operators-cnpg,operators-storage-autoscaler|"
  ...
  "monitoring-ingress-auth|oci://${HARBOR}/fleet/monitoring-ingress-auth|${BUNDLE_VERSION}|monitoring|monitoring-ingress-auth|monitoring-prometheus-stack,operators-storage-autoscaler|"
  ...
  "harbor-manifests|oci://${HARBOR}/fleet/harbor-manifests|${BUNDLE_VERSION}|harbor|harbor-manifests|harbor-core,operators-storage-autoscaler|"

  # 35-backup: dedicated MinIO for cross-cluster backups
  "backup-minio|oci://${HARBOR}/fleet/backup-minio|${BUNDLE_VERSION}|backup|backup-minio|pki-vault-bootstrap-store|"
  "backup-init|oci://${HARBOR}/fleet/backup-init|${BUNDLE_VERSION}|backup|backup-init|backup-minio,pki-vault-bootstrap-store|"
  "backup-manifests|oci://${HARBOR}/fleet/backup-manifests|${BUNDLE_VERSION}|backup|backup-manifests|backup-minio,operators-storage-autoscaler|"
  ...
  "gitops-argocd-manifests|oci://${HARBOR}/fleet/gitops-argocd-manifests|${BUNDLE_VERSION}|argocd|gitops-argocd-manifests|gitops-argocd,operators-storage-autoscaler|"
  ...
  "gitlab-manifests|oci://${HARBOR}/fleet/gitlab-manifests|${BUNDLE_VERSION}|gitlab|gitlab-manifests|gitlab-ready,gitlab-init,operators-gateway-api-crds,operators-storage-autoscaler|"
...
# ============================================================
# Apply BUNDLE_PREFIX to parsed HELMOP_DEFS fields
# ============================================================
# Prefixes name, dependsOn refs, and raw bundle OCI paths.
# Upstream chart OCI paths (/helm/) are NOT prefixed (shared).
# Must be called after IFS='|' read of a HELMOP_DEFS entry.
# Sets: name, depends (if present), oci_repo (if raw bundle).
apply_helmop_prefix() {
  [[ -z "${PREFIX}" ]] && return 0

  # 1. Prefix the HelmOp name
  name="${PREFIX}${name}"

  # 2. Prefix each comma-separated dependency
  if [[ -n "${depends:-}" ]]; then
    # shellcheck disable=SC2001  # sed needed to prefix each comma-separated value
    depends=$(echo "${depends}" | sed "s/[^,]*/${PREFIX}&/g")
  fi

  # 3. Prefix raw bundle OCI paths (fleet/ project), not upstream charts (helm/ project)
  if [[ "${oci_repo:-}" == *"/fleet/"* ]]; then
    oci_repo="${oci_repo/\/fleet\//\/fleet\/${PREFIX}}"
  fi
}
...
")
  ...
  if [[ "${name}" == "${PREFIX}harbor-core" ]]; then
    local valkey_pw=""
    local cluster_id
        ...
        diff: {
          comparePatches: [
            ...
            },
            {
              apiVersion: "batch/v1",
              kind: "Job",
              jsonPointers: ["/status", "/metadata/labels/batch.kubernetes.io~1controller-uid", "/metadata/labels/batch.kubernetes.io~1job-name", "/metadata/labels/controller-uid", "/metadata/labels/job-name"]
            },
            {
              apiVersion: "postgresql.cnpg.io/v1",
              kind: "Cluster",
              jsonPointers: ["/spec/storage/size"]
            }
          ]
        },
...
purge_harbor_oci() {
  ...
  local bundle_names=(
    ...
    backup-minio backup-init backup-manifests
  ...
  for repo in "${bundle_names[@]}"; do
    local prefixed_repo="${PREFIX}${repo}"
    local code
    code=$(curl -sk -o /dev/null -w "%{http_code}" \
      -u "${harbor_user}:${harbor_pass}" \
      -X DELETE "https://${HARBOR}/api/v2.0/projects/fleet/repositories/${prefixed_repo}")
    if [[ "${code}" == "200" ]]; then
      log_ok "Deleted Harbor repo: fleet/${prefixed_repo}"
    elif [[ "${code}" == "404" ]]; then
      log_info "Not found (skip): fleet/${prefixed_repo}"
    else
      log_warn "Failed to delete fleet/${prefixed_repo} (HTTP ${code})"
    fi
  done
...
heal_stuck_bundles() {
  ...
  while (( elapsed < max_wait )); do
    ...
    for entry in "${HELMOP_DEFS[@]}"; do
      ...
      # NOTE: HEAL_EXCLUSIONS uses unprefixed names (matches raw HELMOP_DEFS entries)
      # Prefix is applied AFTER this check
      local skip=false
    ...
    if (( elapsed > 0 && elapsed % 60 == 0 )); then
      ...
      for stuck_name in "${stuck_bundles[@]}"; do
        ...
        for def_entry in "${HELMOP_DEFS[@]}"; do
          ...
          if [[ "${PREFIX}${def_name}" == "${stuck_name}" ]]; then
      ...
      if [[ -s "${ds_kc_heal}" ]]; then
        for stuck_name in "${stuck_bundles[@]}"; do
          ...
          for def_entry in "${HELMOP_DEFS[@]}"; do
            local dep_name dep_release dep_depends
            IFS='|' read -r dep_name _ _ dep_ns dep_release dep_depends _ <<< "${def_entry}"
            local dep_name_raw="${dep_name}"
            dep_name="${PREFIX}${dep_name}"
            if [[ -n "${dep_depends}" && -n "${PREFIX}" ]]; then
              # shellcheck disable=SC2001  # sed needed to prefix each comma-separated value
              dep_depends=$(echo "${dep_depends}" | sed "s/[^,]*/${PREFIX}&/g")
            fi
            if [[ "${dep_depends}" == *"${stuck_name}"* && "${dep_name}" != "${stuck_name}" ]]; then
              # Delete Helm releases for this dependent bundle on downstream cluster
              # Use actual release name from HELMOP_DEFS (not prefixed bundle name)
              local release_name="${dep_release:-${dep_name_raw}}"
...
# Skip if --ci-mode and rendered/ already exists (push stage already rendered)
if [[ "${CI_MODE}" == "true" && -d "${FLEET_DIR}/rendered" ]]; then
  log_info "CI mode: using pre-rendered templates from push stage"
else
  log_info "Rendering templates..."
  _render_env_args=()
  if [[ "${ENV_FILE}" != "${FLEET_DIR}/.env" ]]; then
    _render_env_args=("--env" "${ENV_FILE}")
  fi
  "${SCRIPT_DIR}/render-templates.sh" "${_render_env_args[@]+"${_render_env_args[@]}"}"
  unset _render_env_args
fi
...
if [[ -n "${PREFIX}" ]]; then
  log_info "BUNDLE_PREFIX='${PREFIX}' — all HelmOp names, dependencies, and raw bundle OCI paths will be prefixed"
fi
...
path \"kv/data/services/harbor/helm-sync\" { capabilities = [\"read\"] }
path \"kv/data/services/ci/github-mirror\" { capabilities = [\"read\"] }
EOF
...
cleanup_completed_init_jobs() {
  ...
  declare -A JOB_MANIFEST_FILES=(
    ...
    ["backup/backup-init"]="35-backup/backup-init/manifests/backup-init-job.yaml"
...
for entry in "${HELMOP_DEFS[@]}"; do
  ...
  # Apply BUNDLE_PREFIX to name, dependencies, and raw bundle OCI paths
  apply_helmop_prefix
