...
#   ./render-templates.sh --env .env.rke2-test    # Use a specific environment file
...
# --- Load environment (supports --env <file> override) ---
# shellcheck source=lib/env-loader.sh
source "${SCRIPT_DIR}/lib/env-loader.sh"
[[ -f "${ENV_FILE}" ]] || die "Environment file not found at ${ENV_FILE}. Copy .env.example to .env and fill in values."
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: $0 [--check] [--diff] [--env <file>]"
