...
$ docker push harbor.dev.aegisgroup.ch/forge/svc-forge:abc1234
```
...
deploy-dev:
  ...
  script:
    - |
      ...
      git clone https://${CI_JOB_TOKEN_USER}:${CI_JOB_TOKEN}@gitlab.aegisgroup.ch/platform/platform-deployments.git
      cd platform-deployments

      # Update the image tag in your app's kustomization
      cd dev/forge/svc-forge
      kustomize edit set image CHANGEME_IMAGE=harbor.dev.aegisgroup.ch/forge/svc-forge:${CI_COMMIT_SHORT_SHA}
...
2. **Add Kustomize overlays** (platform team provides a template):
   ...
   images:
   - name: CHANGEME_IMAGE
     newName: harbor.dev.aegisgroup.ch/forge/svc-new-app
     newTag: latest
   ```
