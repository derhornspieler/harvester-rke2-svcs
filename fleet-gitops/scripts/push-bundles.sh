...
#
# Usage: ./push-bundles.sh [--version 1.0.0] [--env .env.rke2-test]
#
# Prerequisites:
#   - helm CLI
...
BUNDLES=(
  ...
  "35-backup/minio-backup:backup-minio"
  "35-backup/backup-init:backup-init"
  "35-backup/backup-manifests:backup-manifests"
  ...
  "60-cicd-onboard/onboard-rbac:onboard-rbac"
...
usage() {
  cat <<EOF
Usage: $(basename "$0") [--version <semver>] [--env <file>]

Options:
  --version    Chart version to use (default: 1.0.0)
  --env <file> Use a specific environment file (default: .env)
...
main() {
  ...
  "${SCRIPT_DIR}/render-templates.sh" "${env_args[@]+"${env_args[@]}"}"
