...
# --- CNPG instance counts (default: 3 for HA) ---
export CNPG_INSTANCES_KEYCLOAK="${CNPG_INSTANCES_KEYCLOAK:-3}"
export CNPG_INSTANCES_GRAFANA="${CNPG_INSTANCES_GRAFANA:-3}"
export CNPG_INSTANCES_HARBOR="${CNPG_INSTANCES_HARBOR:-3}"
export CNPG_INSTANCES_GITLAB="${CNPG_INSTANCES_GITLAB:-3}"

# --- CNPG PVC sizes ---
export CNPG_STORAGE_KEYCLOAK="${CNPG_STORAGE_KEYCLOAK:-10Gi}"
export CNPG_STORAGE_GRAFANA="${CNPG_STORAGE_GRAFANA:-5Gi}"
export CNPG_STORAGE_HARBOR="${CNPG_STORAGE_HARBOR:-20Gi}"
export CNPG_STORAGE_GITLAB="${CNPG_STORAGE_GITLAB:-50Gi}"

# --- MinIO PVC size per replica ---
export MINIO_PVC_SIZE="${MINIO_PVC_SIZE:-100Gi}"

# --- Monitoring PVC sizes ---
export PROMETHEUS_PVC_SIZE="${PROMETHEUS_PVC_SIZE:-50Gi}"
export LOKI_PVC_SIZE="${LOKI_PVC_SIZE:-50Gi}"

# --- Backup MinIO PVC size ---
export BACKUP_MINIO_PVC_SIZE="${BACKUP_MINIO_PVC_SIZE:-50Gi}"
...
# --- Backup service ---
export BACKUP_FQDN="${BACKUP_FQDN:-backup.${DOMAIN}}"
export BACKUP_TLS_SECRET="${BACKUP_FQDN//./-}-tls"
...
# --- Pause image (overprovisioning pods) ---
export IMAGE_PAUSE="${IMAGE_PAUSE:-registry.k8s.io/pause:3.10}"

# --- Derived image repo/tag splits (for Helm charts that need separate fields) ---
export IMAGE_VAULT_REPO="${IMAGE_VAULT%:*}"
export IMAGE_VAULT_TAG="${IMAGE_VAULT##*:}"
export IMAGE_VALKEY_REPO="${IMAGE_VALKEY%:*}"
export IMAGE_VALKEY_TAG="${IMAGE_VALKEY##*:}"
export IMAGE_HAPROXY_REPO="${IMAGE_HAPROXY%:*}"
export IMAGE_HAPROXY_TAG="${IMAGE_HAPROXY##*:}"
...
${IMAGE_PAUSE}
${IMAGE_VAULT_REPO} ${IMAGE_VAULT_TAG}
${IMAGE_VALKEY_REPO} ${IMAGE_VALKEY_TAG}
${IMAGE_HAPROXY_REPO} ${IMAGE_HAPROXY_TAG}
${LABEL_PREFIX}
...
${CNPG_INSTANCES_KEYCLOAK} ${CNPG_INSTANCES_GRAFANA} ${CNPG_INSTANCES_HARBOR} ${CNPG_INSTANCES_GITLAB}
${CNPG_STORAGE_KEYCLOAK} ${CNPG_STORAGE_GRAFANA} ${CNPG_STORAGE_HARBOR} ${CNPG_STORAGE_GITLAB}
${MINIO_PVC_SIZE} ${PROMETHEUS_PVC_SIZE} ${LOKI_PVC_SIZE} ${BACKUP_MINIO_PVC_SIZE}
${BUNDLE_VERSION}'
