...
- **OWASP A03 (Injection)**: Check all input handling paths for:
  ...
  - Path traversal
- **Domain Substitution**: Verify `_subst_changeme()` is used correctly — it only handles `example.com` and `CHANGEME_*` patterns, NOT `changeme.dev`. Flag any `changeme.dev` literals.
...
- **Kubernetes Hardening**:
  ...
  - All images sourced from Harbor (`harbor.example.com`) — never direct pulls from Docker Hub, GHCR, or quay.io.
- **Terraform Security**:
...
5. **NEVER approve images pulled from registries other than Harbor (`harbor.example.com`).**
...
- `_subst_changeme()` only handles `example.com` ↔ `CHANGEME_*` — `changeme.dev` is a known gap
