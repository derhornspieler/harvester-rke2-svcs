...
- **Container Registry**: Harbor (`harbor.example.com`) as pull-through cache — ALL images must route through Harbor, never direct pulls from Docker Hub, GHCR, or quay.io
...
- Domain: `example.com` (sanitized to `changeme.dev` for public repo)
...
6. **Domain check**: Ensure `example.com` is used in operational configs, `changeme.dev` or `CHANGEME_*` placeholders in public/template configs. Never hardcode `changeme.dev` where `_subst_changeme()` should handle substitution.
