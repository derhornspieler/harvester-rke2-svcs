...
### Domain and Registry
- Domain: `aegisgroup.ch` (sanitized to `changeme.dev` in public repo)
- Registry: `harbor.aegisgroup.ch` (pull-through cache for all images)
...
- `changeme.dev` NOT matched by `_subst_changeme()` — only `aegisgroup.ch`
...
- [ ] Public repo sanitization considered (`changeme.dev` vs `aegisgroup.ch`)
